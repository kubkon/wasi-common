...
pub fn ciovec_to_host<'a>(ciovec: &'a __wasi_ciovec_t) -> io::IoSlice<'a> {
    let slice = unsafe { slice::from_raw_parts(ciovec.buf as *const u8, ciovec.buf_len) };
    io::IoSlice::new(slice)
}

pub fn ciovec_to_host_mut<'a>(ciovec: &'a mut __wasi_ciovec_t) -> io::IoSliceMut<'a> {
    let slice = unsafe { slice::from_raw_parts_mut(ciovec.buf as *mut u8, ciovec.buf_len) };
    io::IoSliceMut::new(slice)
}

pub fn iovec_to_host<'a>(iovec: &'a __wasi_iovec_t) -> io::IoSlice<'a> {
    let slice = unsafe { slice::from_raw_parts(iovec.buf as *const u8, iovec.buf_len) };
    io::IoSlice::new(slice)
}

pub fn iovec_to_host_mut<'a>(iovec: &'a mut __wasi_iovec_t) -> io::IoSliceMut<'a> {
    let slice = unsafe { slice::from_raw_parts_mut(iovec.buf as *mut u8, iovec.buf_len) };
    io::IoSliceMut::new(slice)
}
