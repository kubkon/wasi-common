...
pub(crate) fn path_create_directory(dirfd: File, path: String) -> Result<()> {
    ...
    match unsafe { mkdirat(dirfd.as_raw_fd(), path_cstr.as_ptr(), 0o777) } {
        0 => Ok(()),
...
pub(crate) fn path_filestat_get(
    ...
    let filestat = fstatat(dirfd.as_raw_fd(), path.as_str(), atflags)
...
pub(crate) fn path_unlink_file(dirfd: File, path: String) -> Result<()> {
    ...
    match unsafe { unlinkat(dirfd.as_raw_fd(), path_cstr.as_ptr(), 0) } {
        0 => Ok(()),
        _ => {
...
pub(crate) fn path_remove_directory(dirfd: File, path: String) -> Result<()> {
    ...
    // nix doesn't expose unlinkat() yet
    match unsafe { unlinkat(dirfd.as_raw_fd(), path_cstr.as_ptr(), AT_REMOVEDIR) } {
        0 => Ok(()),
